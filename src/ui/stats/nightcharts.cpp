...
{
    qSort(pieces.begin(), pieces.end(), sortPieChartPieces);
...
void Nightcharts::drawLegend(QPainter *painter)
{
    ...
    case Nightcharts::Vertical:
    {
        ...
        int offset = cH - pieces.size()*(painter->fontMetrics().height()+dist);
        ...
        for (int i=pieces.size()-1;i>=0;i--)
        {
            ...
            QString text = pieces[i].pname + " - " + QString::number(pieces[i].pPerc)+"%";

            float x = lX+dist;
            float y = lY+dist+offset/2+i*(painter->fontMetrics().height()+dist);
            painter->drawRect(x,y,painter->fontMetrics().height(),painter->fontMetrics().height());
            //painter->drawRect(x+painter->fontMetrics().height()+dist,y, painter->fontMetrics().width(text), painter->fontMetrics().height());
            painter->drawText(QRect(x+painter->fontMetrics().height()+dist,y, painter->fontMetrics().width(text), painter->fontMetrics().height()),Qt::AlignLeft | Qt::AlignVCenter, text);
        }
        break;
    }
...
void NightChartsWidget::paintEvent(QPaintEvent *event)
{
    ...
    pieChart.setCords(0,this->height()/4,this->width()/2,this->height()/2);

    foreach(QString text, m_values.keys()) {
        pieChart.addPiece(text, m_values.value(text).at(1).value<QColor>(), m_values.value(text).at(0).toDouble());
    }
...
void NightChartsWidget::addValue(QString text, double value, QColor color)
{
    m_values.insert(text, QList<QVariant>() << QVariant(value) << color);
}


bool sortPieChartPieces(pieceNC one, pieceNC two)
{
    return one.pPerc > two.pPerc;
}
